...
private:
    ...
    void saveRec(Node *node, ofstream &file)
    {
        if (node)
        {
            file << node->ktp->NIK << "|" << node->ktp->nama << "|" << node->ktp->alamat << "|" << node->ktp->tanggalLahir << "|" << node->ktp->isAdmin << endl;
...
public:
    ...
    void registerUser()
    {
        ...
        k->isAdmin = false; // User biasa bukan admin
        root = insert(root, k);
        saveToFile();
    ...
    void loadUsers()
    {
        ...
        while (getline(file, line))
        {
            ...
            if (c >= 4)
            {
                ...
                k->isAdmin = (c == 5 && parts[4] == "1"); // Baca field isAdmin
